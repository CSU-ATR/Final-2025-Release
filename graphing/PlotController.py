...
class PlotController:
    ...
    def update_viable_axes(self):
        axes_data = self.plotter_variables.data[self.plotter_variables.axis_columns]
        non_constant_columns = axes_data.columns[axes_data.nunique() > 1]
    ...
    def initialize_data(self, data: pd.DataFrame):
        ...
        if self.plotter_variables.current_frequency not in self.plotter_variables.available_frequencies:
            self.plotter_variables.current_frequency = self.plotter_variables.available_frequencies[0]
        self.limit_data_by_current_frequency()
        self.set_response_values_to_raw_magnitude()

    # ---------------------------
    # STL Export API (GUI should call THIS)
    # ---------------------------
    def export_stl(self, filepath: str):
        """
        Export the currently displayed spherical surface to STL.

        Requirements:
        - Plot type must be Spherical
        - interpolation must be > 0 (surface mode)
        - surface must have been plotted at least once
        """
        if self.plotter_type != "Spherical":
            raise RuntimeError("STL export is only supported for Spherical plots.")

        if int(self.plotter_variables.interpolation) == 0:
            raise RuntimeError("Set interpolation > 0 (surface mode) before exporting STL.")

        if self.plotter is None or not isinstance(self.plotter, PlotterSpherical):
            # ensure plotter exists
            self.plot_spherical()

        return self.plotter.export_current_surface_to_stl(filepath)
